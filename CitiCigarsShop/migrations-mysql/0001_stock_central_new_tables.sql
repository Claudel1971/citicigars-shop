...
-- Toutes les NOUVELLES tables Stock Central sont forcées en utf8mb4_unicode_ci.
-- Raison: la DB historique mélange latin1 (products/product_images/technical_sheets)
-- et utf8mb4 (bundles/bundle_items). Les clés des nouvelles tables doivent avoir
-- une collation commune et prévisible pour que leurs FK internes soient valides.
...
ALTER TABLE `accessories` ADD CONSTRAINT `accessories_sku_skus_sku_fk` FOREIGN KEY (`sku`) REFERENCES `skus`(`sku`) ON DELETE no action ON UPDATE no action;
--> statement-breakpoint
ALTER TABLE `dna_availability_watch` ADD CONSTRAINT `dna_availability_watch_lead_id_dna_leads_id_fk` FOREIGN KEY (`lead_id`) REFERENCES `dna_leads`(`id`) ON DELETE no action ON UPDATE no action;
--> statement-breakpoint
ALTER TABLE `pack_size_config` ADD CONSTRAINT `pack_size_config_sku_skus_sku_fk` FOREIGN KEY (`sku`) REFERENCES `skus`(`sku`) ON DELETE no action ON UPDATE no action;
--> statement-breakpoint
ALTER TABLE `stock_balances` ADD CONSTRAINT `stock_balances_sku_skus_sku_fk` FOREIGN KEY (`sku`) REFERENCES `skus`(`sku`) ON DELETE no action ON UPDATE no action;
--> statement-breakpoint
ALTER TABLE `stock_movements` ADD CONSTRAINT `stock_movements_sku_skus_sku_fk` FOREIGN KEY (`sku`) REFERENCES `skus`(`sku`) ON DELETE no action ON UPDATE no action;
--> statement-breakpoint
CREATE INDEX `idx_cigar_catalog_mlv` ON `cigar_catalog` (`marque`,`ligne`,`vitole`);
--> statement-breakpoint
CREATE INDEX `idx_dna_watch_status` ON `dna_availability_watch` (`status`);
--> statement-breakpoint
CREATE INDEX `idx_dna_leads_profile` ON `dna_leads` (`dna_profile_id`);
--> statement-breakpoint
CREATE INDEX `idx_pack_size_config_sku` ON `pack_size_config` (`sku`);
--> statement-breakpoint
CREATE INDEX `idx_stock_movements_history` ON `stock_movements` (`sku`,`type`,`balance_field`,`created_at`);
--> statement-breakpoint
CREATE INDEX `idx_stock_movements_group` ON `stock_movements` (`group_id`);
--> statement-breakpoint
