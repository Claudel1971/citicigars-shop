...
## Full Business Context, Product Vision & Build Specification — V2.1

**Confidentiel — document de travail — 3 septembre 2026**

**Changelog V2.0 → V2.1 (révision Claude) :** ajout de la Partie 0 (contexte factuel manquant : identité, positionnement, partenaires réels, doctrine de prix, devise, socle technique existant, état réel du chantier Cigar DNA) ; correction de l'identifiant canonique erroné `CTGxxxxxx` en §8.1 et §X2.1 (contredisait ID_CONVENTIONS.md). Le reste du contenu V2.0 est inchangé et n'a pas été revérifié ligne à ligne au-delà de ces points.
...
# PARTIE 0 — QUI EST CITICIGARS, CONCRÈTEMENT

*(Ajouté en révision V2.1 — cette partie précise les faits qui manquaient à la V2.0. Elle n'est pas optionnelle : plusieurs points ci-dessous sont des invariants au même titre que la doctrine de stock/finance déjà exposée plus loin.)*

## 0.1 Identité

CitiCigars opère sous **ETS Citi Businesses**, à Douala, Cameroun — premier distributeur structuré de cigares premium au Cameroun. Le fondateur, **Claudel**, cumule les rôles de CEO, CFO, CMO et responsable sourcing, avec plus de 20 ans d'expérience en finance et commerce international. Il opère en partie depuis Montréal. Le code de la plateforme e-commerce vit sur GitHub : `Claudel1971/citicigars-shop`.

## 0.2 Positionnement à trois paliers (invariant — ne pas réinventer)

- **Prestige** — lieu : Le Babylone (niveau Président, format intimiste).
- **Executive** — lieu : Le Grey (afterworks).
- **Discovery** — troisième lieu encore à déterminer.

L'architecture d'expérience et le modèle de données doivent pouvoir refléter cette segmentation existante, pas en proposer une autre.

## 0.3 Trois piliers de marque (invariant)

- Authenticité garantie
- Conservation irréprochable
- Juste prix

Ce sont les termes de marque réels ; toute promesse générée doit s'y ancrer plutôt que reformuler autre chose.

## 0.4 Partenaires réels déjà actifs

- **Le Grey Lounge & Bar** — partenaire de lieu (palier Executive).
- **Martell / Pernod Ricard** — co-activation spiritueux envisagée, **conditionnée** à la conformité avec la **Loi camerounaise N°2006/018** (publicité sur le tabac) — point de blocage binaire, ne jamais présenter cette activation comme acquise.
- **Potolo Logistics** et **Be Well Agency** — partenaires opérations.
- **Partenaire commercial en République du Congo** — activations retail transfrontalières, encadrées par le Code des douanes CEMAC n°05/19-UEAC-010-CM-33.

## 0.5 Sourcing et fabricants

- Contacts fabricant ciblés : **Lissette et Ernesto Perez-Carrillo Jr.** (Casa Carrillo).
- Pivot de sourcing à l'étude : vers **Oliva** et **Casa Carrillo** en conditions de gros direct, en s'éloignant de **My Father**.
- ~93 références actuellement retenues pour approvisionnement (issues du fichier de référence « Top 25 normalisé », colonne Classification Appro).

## 0.6 Doctrine de prix (invariant)

- Benchmark de référence : prix duty-free **Paris CDG**, que les clients connaissent et comparent activement.
- Une prime de **15 à 25 %** au-dessus de Paris est la fourchette de travail ; en dessous, cela déclenche des doutes clients sur l'authenticité.
- Trois définitions de marge à ne **jamais** confondre dans le modèle de données ou les analytics : marge de gestion, marge commerciale OHADA, marge produit par SKU.

## 0.7 Devise et change (invariant)

- FCFA (XAF) est la monnaie primaire ; USD pour le sourcing.
- Taux de travail fixe utilisé en interne : **1 USD = 660 FCFA**.

## 0.8 Socle technique déjà en place (à préserver, pas à réinventer)

- Frontend React avec IndexedDB (stockage local), routage Wouter.
- Backend Node.js/Express, base MySQL hébergée sur WHC.
- API hébergée sur Render ; frontend statique sur WHC.
- CDN Cloudinary pour les images produits.
- Monitoring UptimeRobot (ping toutes les 14 minutes sur l'API).
- `staging.citicigars.com` — environnement de test actuel, notamment pour l'intégration du Cigar DNA avant bascule en production, et l'occasion prise pour revoir l'architecture.
- Repo : `Claudel1971/citicigars-shop`.

Le moteur Phase 2 décrit plus loin dans ce document s'inscrit dans (ou à côté de) ce socle — Replit doit clarifier explicitement, dans son Architecture & Product Challenge Memo, comment son architecture proposée s'articule avec l'existant ci-dessus plutôt que de le laisser implicite.

## 0.9 Convention d'identifiants déjà figée (voir ID_CONVENTIONS.md)

- Customers : `CTCG-CUST-XXXXXX`
- Sales : `CTCG-SALE-XXXXXX`
- Suppliers : `CTCG-SUPP-XXXXXX`
- SKU : `CTCG-NI-XXXXXX`, `CTCG-RD-XXXXXX`, `CTCG-BDL-XXXXXX`, `CTCG-ACC-XXX`, `CTCG-SRV-XXX` (migration `CTG→CTCG` déjà testée et validée, `ON UPDATE CASCADE` confirmé sur les tables filles ; mapping exhaustif dans `sku_legacy_to_ctcg.csv`).

**Important — correction :** le préfixe `CTG` (une seule lettre C) est un préfixe **legacy retiré**. Les sections §8.1 et §X2.1 plus loin dans ce document désignent par erreur `CTGxxxxxx` comme l'identifiant canonique du Cigar ID. Une note de correction est insérée à ces deux endroits ci-dessous — la forme exacte du nouvel identifiant (distinct du SKU) reste à confirmer par Claudel, mais elle doit suivre la famille `CTCG-*`, jamais réutiliser `CTG` seul.

## 0.10 Cigar DNA — où nous en sommes réellement

- Un agent de recherche de références (Golden Dataset) est en cours de validation sur 3 cas pilotes : CC-T001 (Oliva Serie V Melanio Maduro), CC-T002 (Casa Carrillo Core Plus Maduro), CC-T003 (Arturo Fuente Don Carlos No. 2) — phase de comparaison contre un Oracle gelé, arbitrages en cours.
- Le fichier « Top 25 normalisé » (classements Cigar Aficionado / Cigar Journal 2004-2025) sert de palier de preuve interne à haute confiance pour les cigariers personnellement revus par Claudel.
- Une taxonomie d'identité client est **en construction, pas encore figée** : 4 racines — **Le Velouté, Le Boisé, Le Gourmand, Le Fauve** — combinées à une matrice puissance × intensité aromatique. Ne pas la considérer comme un livrable fini ; le modèle de données doit rester extensible sur ce point.
- Une dimension **Combustion** (qualité du tirage, comportement de combustion, texture et couleur de cendre) est en cours d'évaluation de faisabilité (couverture documentaire des sources) — **pas encore décidée**. Ne pas l'ajouter au modèle de données tant que ce n'est pas confirmé.

## 0.11 Principes de croissance déjà établis

- Sortir du premier cercle du fondateur — construire une acquisition de marque indépendante de son réseau personnel (cohérent avec la section « Sortir du premier cercle » plus loin dans ce document).
- Modèles cash-immédiat, zéro-crédit uniquement.
- Événements de dégustation/découverte payants préférés à l'échantillonnage gratuit.
- Un modèle freemium est déjà intégré à la plateforme : catalogue de base gratuit, fiches techniques expertes en accès premium (ex. section « Analyse experte » déjà en place sur staging, actuellement gated à 9 900 FCFA/mois).
...
1. **mieux acheter** ;
2. **mieux stocker et tracer** ;
3. **mieux vendre** ;
4. **mieux connaître nos clients** ;
5. **mieux les relancer** ;
6. **mieux recommander** ;
7. **mieux encaisser** ;
8. **mieux mesurer la marge et le capital immobilisé** ;
9. **mieux exploiter les événements, partenariats et opportunités corporate** ;
10. **prendre de meilleures décisions que la concurrence**.
...
> **Correction V2.1 :** la V2.0 indiquait ici `CTGxxxxxx`. Ce préfixe est **legacy et retiré** (voir §0.9) — ne pas l'utiliser. La forme exacte du Cigar ID reste à confirmer par Claudel, mais elle doit suivre la famille `CTCG-*` déjà en usage pour tous les autres identifiants du système, et rester distincte du SKU commercial (§8.4).

Le **Cigar ID** représente l’identité canonique d’un cigare dans notre référentiel de connaissance.
...
1. manufacturer / source officielle ;
2. sources spécialisées reconnues ;
3. sources secondaires contrôlées.
...
2. **Quelle est notre relation avec elle ?**
3. **Que savons-nous de ses goûts, besoins et comportements ?**
4. **Que s’est-il passé entre nous ?**
5. **Quelle est la prochaine meilleure action ?**
...
1. **Accueil / Command Center**
2. **Ventes**
3. **Clients**
4. **Stock**
5. **Achats & Réceptions**
6. **Trésorerie opérationnelle**
7. **Analytics**
8. **Intelligence / Actions**
...
1. anomalie / incohérence critique ;
2. rupture ;
3. stock faible ;
4. client totalement / fortement réservé ;
5. PO en retard ;
6. créances en retard ;
7. stock ancien / dormant à enjeu financier ;
8. lot ancien ;
9. opportunité de réapprovisionnement ;
10. qualité de donnée historique.
...
1. code implémenté ;
2. lint/typecheck/build propres ;
3. tests automatisés ;
4. scénario manuel ;
5. capture/preuve visuelle pour UI ;
6. contrôles DB/invariants pertinents ;
7. commit Git ;
8. documentation de continuité mise à jour ;
9. aucun secret commité ;
10. compte rendu structuré.
...
3. expliquer l’impact sur les invariants ;
4. attendre l’arbitrage avant mutation irréversible.
...
1. Quelles parties du frontend actuel vaut-il mieux jeter plutôt que refactorer ?
2. Quels services transactionnels Phase 2 peuvent être réutilisés tels quels ?
3. Quels endpoints/read models manquent pour une excellente UX ?
4. Quelle architecture de dashboard/drill-down évite des requêtes lentes ?
5. Comment modéliser Cash Journals sans perturber `orders` historiques ?
6. Comment introduire landed cost et COGS sans réécrire la vérité historique ?
7. Quel modèle de bundle/sampler est le plus sûr ?
8. Quelle stratégie de reversal protège le ledger ?
9. Quelle stratégie PWA/low-connectivity est réaliste sans casser l’atomicité ?
10. Comment architecturer Owner Copilot pour que chaque chiffre soit sourcé par un tool déterministe ?
11. Quelles données de Master Gestion peuvent être importées comme faits commerciaux sans créer de faux faits stock ?
12. Quel plan de migration permet de retirer Master Gestion sans big bang ?
...
> **Contrainte d'exécution non négociable : tu codes exclusivement en mode FREE (aucun agent/mode payant). Si une tâche demandée ne peut pas être réalisée en mode FREE, arrête-toi et signale-le explicitement plutôt que de basculer en mode payant.**

## Avant de coder

1. Lis intégralement ce cahier des charges.
2. Lis `PHASE2_STOCK_TRACEABILITY_HANDOFF.md` et les migrations/services Phase 2.
3. Cartographie l’architecture existante.
4. Produis un plan R0–R7 en indiquant pour chaque partie : **KEEP / REFACTOR / REBUILD UI / EXTEND / NEW**.
5. Challenge les 12 questions de la section 30.
6. Signale toute divergence nécessaire avant de modifier un invariant ou le schéma.
7. Ne touche jamais la production.
...
> **Correction V2.1 :** voir §0.9 et §8.1 — ne pas utiliser `CTGxxxxxx` (préfixe legacy retiré). Préserver un identifiant canonique du cigare distinct du SKU, dans la famille `CTCG-*`, forme exacte à confirmer par Claudel.
...
1. compréhension de CitiCigars en 10 à 20 points ;
2. forces du système actuel ;
3. limites UX actuelles ;
4. architecture proposée ;
5. domaines/modules ;
6. navigation proposée ;
7. modèle CRM proposé ;
8. modèle Cigar ID / Product / SKU ;
9. intégration DNA ;
10. corporate/club/events future-proofing ;
11. architecture CMS ;
12. analytics et drill-down strategy ;
13. agent strategy ;
14. décisions à préserver ;
15. risques ;
16. éléments du cahier qu’il challengerait et pourquoi ;
17. phasage proposé ;
18. démonstration de ce qu’il compte faire pour nous « épater ».
