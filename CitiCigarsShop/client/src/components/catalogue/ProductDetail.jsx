...
import { ShoppingCart, Heart, Share2, Wind, Thermometer, MapPin, Info } from 'lucide-react';
...
const ProductDetail = ({ product, isOpen, onClose }) => {
  ...
  const [selectedFormat, setSelectedFormat] = React.useState('unitaire'); // unitaire, pack, boite

  if (!product) return null;

  // Determine active prices based on format
  const getPrice = (fmt) => {
    let base, promoObj;
    switch(fmt) {
      case 'pack': base = product.prixPack; promoObj = product.promotions.pack; break;
      case 'boite': base = product.prixBoite; promoObj = product.promotions.boite; break;
      default: base = product.prixUnitaire; promoObj = product.promotions.unitaire;
    }
    const final = promoObj.actif ? calculateDiscountedPrice(base, promoObj.pourcentage) : base;
    return { base, final, isPromo: promoObj.actif, pct: promoObj.pourcentage };
  };
          ...
          <div className="p-6 md:p-8 flex flex-col h-full">
            ...
            <div className="flex justify-between items-start mb-2">
              <div className="flex-1">
                {/* Ligne 1: Marque, Ligne · Pays (MÊME STYLE que carte) */}
                <h3 className="text-xl font-serif font-bold text-primary mb-1 leading-tight">
                  {product.marque}{product.ligne && `, ${product.ligne}`}
              ...
              {/* Wishlist button */}
              <button
                onClick={(e) => { e.stopPropagation(); /* toggleWishlist logic */ }}
            ...
            <Tabs defaultValue="tasting" className="flex-1 mb-8">
              ...
              <TabsContent value="tasting" className="space-y-4 animate-in fade-in slide-in-from-bottom-2">
                <div className="space-y-3">
                  <div className="flex gap-3 items-start">
                    <span className="text-lg">🔥</span>
                    <div>
                      <h5 className="font-bold text-primary text-sm">Premier Tiers</h5>
                      <p className="text-xs text-muted-foreground">Notes boisées, cèdre, touche de poivre blanc.</p>
                    </div>
                  </div>
                  <div className="flex gap-3 items-start">
                    <span className="text-lg">🔥🔥</span>
                    <div>
                      <h5 className="font-bold text-primary text-sm">Deuxième Tiers</h5>
                      <p className="text-xs text-muted-foreground">Évolution vers des arômes de café torréfié et de cuir.</p>
                    </div>
                  </div>
                  <div className="flex gap-3 items-start">
                    <span className="text-lg">🔥🔥🔥</span>
                    <div>
                      <h5 className="font-bold text-primary text-sm">Dernier Tiers</h5>
                      <p className="text-xs text-muted-foreground">Finale puissante, terreuse, avec des notes de cacao amer.</p>
                    </div>
                  </div>
                </div>
              </TabsContent>
              
              <TabsContent value="pairings" className="space-y-4 animate-in fade-in slide-in-from-bottom-2">
                <div className="grid grid-cols-2 gap-4">
                  <div className="bg-accent/30 p-3 rounded-lg">
                    <h5 className="font-bold text-primary text-sm mb-2">🥃 Boissons</h5>
                  ...
                  <div className="bg-accent/30 p-3 rounded-lg">
                    <h5 className="font-bold text-primary text-sm mb-2">🍫 Mets</h5>
                    <ul className="text-xs text-muted-foreground space-y-1">
              ...
              <TabsContent value="details" className="space-y-4 animate-in fade-in slide-in-from-bottom-2">
                 <div className="grid grid-cols-2 gap-y-2 text-sm">
                   ...
                   <span className="font-medium">{product.format.split('x')[1]}</span>
                 </div>
              </TabsContent>
            </Tabs>
            ...
            <div className="mt-auto pt-6 border-t border-border">
              <div className="flex gap-2 mb-4">
                {['unitaire', 'pack', 'boite'].map(fmt => {
                   const p = getPrice(fmt);
                   // Only show if active? Assuming all active for mock
                   return (
                     <button
                       key={fmt}
                     ...
                     >
                       <span className="block capitalize text-xs mb-1">{fmt} {fmt === 'pack' ? `(${product.quantitePack || product.typePack || 5})` : fmt === 'boite' ? `(${product.quantiteBoite || product.qteBoite || 10})` : ''}</span>
                       <span className="font-bold">{formatPrice(p.final)}</span>
                     </button>
                   )
                })}
              </div>
              ...
              <div className="flex items-center justify-between gap-4">
                <div>
                  <div className="text-xs text-muted-foreground uppercase tracking-wider">Prix Total</div>
                  <div className="flex items-baseline gap-2">
                    <span className="text-2xl font-serif font-bold text-primary">{formatPrice(currentPrice.final)}</span>
                    {currentPrice.isPromo && (
                      <span className="text-sm text-muted-foreground line-through decoration-destructive/50">{formatPrice(currentPrice.base)}</span>
                    )}
                  </div>
                </div>
